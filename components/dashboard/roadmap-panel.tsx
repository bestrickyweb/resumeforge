...
export function RoadmapPanel({
  ...
  async function onRegenerate() {
    ...
    if (res.ok && res.roadmap) {
      ...
      setResult({ ok: false, error: res.error ?? 'Could not regenerate roadmap.' })
      toast.error(res.error ?? 'Could not regenerate roadmap.')
    }
  }
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-6">
        ...
        <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            {result?.ok && (
              <Button type="button" variant="outline" size="lg" onClick={onRegenerate} disabled={regenerating}>
                {regenerating ? (
                  <>
